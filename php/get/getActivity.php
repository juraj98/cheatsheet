...
//Select
$sql = 	"(SELECT m.id AS id, 'message' as activityType, m.content AS header, Null AS subject, u.name AS name, u.surname AS surname, m.created AS created, Null AS reminderType, Null AS reminderDate, Null AS postContent FROM messages m, users u, classMembers cm WHERE m.userId=u.id AND cm.userId=" . $userId . " AND m.classId=cm.classId" . ($filterSql ? $filterMessages : "") . ") UNION (SELECT p.id as id, 'post' as activityType, p.name AS header, p.subject AS subject, u.name AS name, u.surname AS surname, p.created AS created, Null AS reminderType, Null AS reminderDate, p.content AS postContent FROM posts p, users u, classMembers cm WHERE p.authorId=u.id AND cm.userId=" . $userId . " AND p.classId=cm.classId" . ($filterSql ? $filterPosts : "") . ") UNION (SELECT r.id as id, 'reminder' as activityType, r.name AS header, r.subject AS subject, u.name AS name, u.surname AS surname, r.created AS created, r.type AS reminderType, r.dateOfReminder AS reminderDate, Null AS postContent FROM reminders r, users u, classMembers cm WHERE r.authorId=u.id AND cm.userId=" . $userId . " AND r.classId=cm.classId" . ($filterSql ? $filterReminders : "") . ") ORDER BY created DESC LIMIT " . mysqli_real_escape_string($conn, $_POST["limit"]) . " OFFSET " . mysqli_real_escape_string($conn, $_POST["offset"]);
